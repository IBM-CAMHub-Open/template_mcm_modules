...
PARAM_CLUSTER_CONSOLE_PORT=8443

# Get script parameters
while test $# -gt 0; do
  [[ $1 =~ ^-c|--cluster ]] && { PARAM_CLUSTER="${2}"; shift 2; continue; };
  [[ $1 =~ ^-h|--consolehost ]] && { PARAM_CLUSTER_CONSOLE_HOST="${2}"; shift 2; continue; };
  [[ $1 =~ ^-p|--consoleport ]] && { PARAM_CLUSTER_CONSOLE_PORT="${2}"; shift 2; continue; };
...
if [ -z "$PARAM_CLUSTER_CONSOLE_HOST" ]; then
	echo "Console host of managed ICP is missing.Failed to register ICP to hub cluster."
	exit 1
fi

if [ -z "$PARAM_CLUSTER_CONSOLE_PORT" ]; then
	echo "Console port of managed ICP is missing. Will use default port 8443."
fi
...
if ! which kubectl; then
	echo "install kubectl from ${PARAM_CLUSTER_CONSOLE_HOST}"
	curl -kLo kubectl-linux-${systemArch} https://${PARAM_CLUSTER_CONSOLE_HOST}:${PARAM_CLUSTER_CONSOLE_PORT}/api/cli/kubectl-linux-amd64	
...
if ! which cloudctl; then
	...
	curl -kLo "cloudctl-linux-${systemArch}" https://${PARAM_CLUSTER_CONSOLE_HOST}:${PARAM_CLUSTER_CONSOLE_PORT}/api/cli/cloudctl-linux-amd64	
