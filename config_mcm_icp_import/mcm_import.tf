...
resource "null_resource" "import_icp" {
  ...
  provisioner "remote-exec" {
    inline = [    
      ...
      "echo /var/lib/registry/mcm_scripts/mcm_import.sh -rh ${var.cluster_docker_registry_server_name} -rp ${var.cluster_docker_registry_server_port} -ri ${var.cluster_docker_registry_server_ip} -cm ${var.cluster_name} -pa ${var.icp_dir} -hs ${var.mcm_controller_server_name} -hu ${var.mcm_controller_admin_user} -mch ${var.man_cluster_on_hub} -u ${var.admin_user} -v ${var.icp_inception_image} -s ${var.icp_server_url}",
      "bash -c '/var/lib/registry/mcm_scripts/mcm_import.sh -ri ${var.cluster_docker_registry_server_ip} -rca ${var.cluster_docker_registry_server_ca_crt} -rh ${var.cluster_docker_registry_server_name} -rp ${var.cluster_docker_registry_server_port} -cm ${var.cluster_name} -pa ${var.icp_dir} -hs ${var.mcm_controller_server_name} -hp ${var.mcm_controller_admin_user_password} -hu ${var.mcm_controller_admin_user} -mch ${var.man_cluster_on_hub} -u ${var.admin_user} -pw ${var.admin_user_password} -s ${var.icp_server_url} -v ${var.icp_inception_image}'"      
