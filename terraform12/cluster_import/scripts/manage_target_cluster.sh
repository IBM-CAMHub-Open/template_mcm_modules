...
function verifyMcmHubClusterInformation() {
    if [ -z "$(echo "${HUB_URL}" | tr -d '[:space:]')" ]; then
        exitOnError "MCM hub-cluster API URL is not available"
    fi
    if [ -z "$(echo "${HUB_ADMIN_USER}" | tr -d '[:space:]')" ]; then
        exitOnError "MCM hub-cluster admin username is not available"
    fi
    if [ -z "$(echo "${HUB_ADMIN_PASSWORD}" | tr -d '[:space:]')" ]; then
        exitOnError "MCM hub-cluster admin password is not available"
    fi
...
function prepareClusterImport() {
    ...
    if [ $? -ne 0 ]; then
        exitOnError "Unable to generate cluster configuration file template"
    fi
    set -e
    ...
    if [ $clusterCount -eq 0 ]; then
        ...
        if [ $? -ne 0 ]; then
            exitOnError "Unable to create the resource for the target cluster within the hub"
        fi
    elif [ $clusterPending -eq 1 ]; then
        echo "Cluster ${CLUSTER_NAME} has previously been created; Import is pending..."
    else
        echo "Cluster ${CLUSTER_NAME} already exists in hub with an unexpected state"
        kubectl get clusters -n ${nameSpace}
        exitOnError "Cluster ${CLUSTER_NAME} already exists in hub with an unexpected state"
    fi
    ...
    if [ $? -ne 0 ]; then
        exitOnError "Unable to generate import file for target cluster"
    fi
    set -e
...
function monitorClusterImport() {
    ...
    if [ "${clusterStatus}" != "Ready" ]; then
        echo "${WARN_ON}Cluster is not ready within the allotted time${WARN_OFF}"
        echo "${WARN_ON}State of target cluster shown below:${WARN_OFF}"
        export KUBECONFIG=${KUBECONFIG_FILE}
        ${WORK_DIR}/bin/kubectl get pods -n multicluster-endpoint
        unset KUBECONFIG
        exitOnError "Cluster is not ready within the allotted time"
    else
